...
import { AdCardListWrapper, AdCardGridWrapper } from './AdCardWrapper';
...
export function AdCard({ item, viewMode = 'grid' }: AdCardProps) {
  ...
  if (viewMode === 'list') {
    return (
      <AdCardListWrapper>
        ...
        <CardContent className="p-4 pl-8 flex-1 flex flex-col">
          <div>
  ...
  return (
    <AdCardGridWrapper>
      <Card
        className="cursor-pointer transition-shadow hover:shadow-md flex flex-col h-full pt-0 pb-4"
        onClick={() => navigate(`/ads/${item.id}`)}
      >
        ...
        {item.needsRevision && (
          <Badge
            variant="outline"
            className="text-orange-500 dark:text-orange-400 border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-950/30 w-fit text-xs mt-auto"
          >
            <span className="w-2 h-2 rounded-full bg-orange-500 dark:bg-orange-400 mr-1.5"></span>
            Требует доработки
          </Badge>
